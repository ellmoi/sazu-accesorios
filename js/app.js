(function () {
  ...
  function productCard(p) {
    const state = inventory(p.stock),
      cls = state.replace(" ", "-");
    return `<article class="product">${p.offer ? `<span class="offer">-${p.offer}%</span>` : ""}<button class="icon fav" data-fav="${p.id}" aria-label="Favorito">♡</button><div class="product-img"><img src="${p.image}" alt="${p.name}" loading="lazy"></div><div class="product-body"><span class="cat">${p.category}</span><h3>${p.name}</h3><span class="rating">★ ${p.rating} · ${p.sales} ventas</span><span class="price">${money(p.price)}</span><span class="wholesale-price">Mayor: ${money(p.wholesale)} desde ${p.min} uds.</span><span class="stock ${cls}">${state} · ${p.stock} disponibles</span><div class="product-actions"><button class="btn btn-primary btn-small" data-add="${p.id}" ${p.stock ? "" : "disabled"}>Agregar</button><button class="btn btn-outline btn-small" data-detail="${p.id}">Ver</button></div></div></article>`;
  }
  function bindProducts() {
    $$("[data-add]").forEach(
      (b) => (b.onclick = () => Cart.add(+b.dataset.add)),
    );
    $$("[data-detail]").forEach(
      (b) => (b.onclick = () => openDetail(+b.dataset.detail)),
    );
    $$("[data-fav]").forEach(
      (b) =>
        (b.onclick = () => {
  ...
  function render() {
    const g = $("#productGrid");
    if (!g || !window.PRODUCTS) return;
    const q = ($("#search")?.value || "").toLowerCase(),
      cat = $("#category")?.value || "",
      state = $("#availability")?.value || "",
      min = +($("#minPrice")?.value || 0),
      max = +($("#maxPrice")?.value || 99999999),
    ...
    g.innerHTML = list.length
      ...
      : '<div class="empty"><h3>No hay resultados</h3><p>Prueba con otros filtros.</p></div>';
    $("#productCount") &&
      ($("#productCount").textContent = `${list.length} productos`);
    bindProducts();
  }
  function openDetail(id) {
    ...
    $("#plus").onclick = () =>
      ($("#detailQty").value = Math.min(p.stock, +$("#detailQty").value + 1));
    $("#detailAdd").onclick = () => Cart.add(id, +$("#detailQty").value);
    $("#detailBuy").onclick = () => {
      Cart.add(id, +$("#detailQty").value);
  ...
  function forms() {
    $("#wholesaleForm")?.addEventListener("submit", (e) => {
      ...
      $("#wholesaleMessage").textContent =
        "¡Solicitud recibida! Un asesor te contactaría en esta demostración.";
      e.target.reset();
    });
    $("#loginForm")?.addEventListener("submit", (e) => {
      ...
      if (
        ...
        Sazu.toast("Revisa tus datos");
        return;
      }
      localStorage.setItem(
        "sazuSession",
        JSON.stringify({ name: "Cliente Demo" }),
      );
      Sazu.toast("Sesión iniciada");
      setTimeout(() => (location.href = "perfil.html"), 500);
    });
    $("#registerForm")?.addEventListener("submit", (e) => {
      e.preventDefault();
      let ok = true;
      $$("[required]", e.target).forEach((i) => {
        const bad = !i.value.trim();
        i.closest(".field")?.querySelector(".error") &&
          (i.closest(".field").querySelector(".error").textContent = bad
            ? "Campo obligatorio"
            : "");
        if (bad) ok = false;
      });
      ...
      if (ok) {
        localStorage.setItem(
          "sazuSession",
          JSON.stringify({ name: $("#firstName").value }),
        );
        Sazu.toast("Registro completado");
        setTimeout(() => (location.href = "perfil.html"), 500);
      }
    });
    $("#logout")?.addEventListener("click", () => {
      localStorage.removeItem("sazuSession");
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    $$("[data-open-cart]").forEach((b) => (b.onclick = () => Cart.open()));
    $$("[data-demo]").forEach(
      (b) =>
        (b.onclick = (e) => {
          e.preventDefault();
          Sazu.toast("Función simulada");
        }),
    );
    $$("[data-close]").forEach(
      (b) =>
        (b.onclick = () => {
          b.closest(".modal,.drawer").classList.remove("open");
    ...
    ["input", "change"].forEach((ev) =>
      $$("#search,#category,#availability,#minPrice,#maxPrice,#sort").forEach(
        (i) => i.addEventListener(ev, render),
      ),
    );
    ...
    if (param && $("#category")) $("#category").value = param;
    forms();
    render();
  });
